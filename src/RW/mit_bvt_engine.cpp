...
mit_bvt_engine::mit_bvt_engine() {

}

mit_bvt_engine::mit_bvt_engine(uint64_t max_trace_length, uint64_t max_number_of_intervals, uint64_t max_number_of_involved_inputs, uint64_t max_ast_nodes_trace_length, uint64_t initial_ast_nodes_trace_length, uint64_t memory_allocation_step_ast_nodes_trace) {
...
void mit_bvt_engine::witness_profile() {
  ...
  for (size_t i = 0; i < input_table.size(); i++) {
    for (size_t j = 0; j < mintervals_los[input_table[i]].size(); j++) {
      print_input_witness(i, j, mintervals_los[input_table[i]][j], mintervals_ups[input_table[i]][j], steps[input_table[i]]);
    }
  }
}
...
void print_execution_info(uint64_t paths, uint64_t total_number_of_generated_witnesses_for_all_paths, uint64_t max_number_of_generated_witnesses_among_all_paths, uint64_t queries_reasoned_by_mit, uint64_t queries_reasoned_by_bvt, bool is_number_of_generated_witnesses_overflowed) {
  std::cout << "\n";
  std::cout << YELLOW "method:= PARTI" << RESET << std::endl;
...
uint64_t mit_bvt_engine::compute_add(uint64_t left_operand_ast_tc, uint64_t right_operand_ast_tc, uint64_t old_ast_tc, uint8_t theory_type, uint8_t symbolic_operands) {
  ...
  if (symbolic_operands == LEFT) {
    ...
    ast_ptr = add_ast_node(ADD, sym_operand_ast_tc, crt_operand_ast_tc, mintervals_los[sym_operand_ast_tc].size(), propagated_minterval_lo, propagated_minterval_up, 1, involved_sym_inputs_cnts[sym_operand_ast_tc], involved_sym_inputs_ast_tcs[sym_operand_ast_tc], theory_type, smt_exprs[old_ast_tc]);
...
uint64_t mit_bvt_engine::compute_sub(uint64_t left_operand_ast_tc, uint64_t right_operand_ast_tc, uint64_t old_ast_tc, uint8_t theory_type, uint8_t symbolic_operands) {
  ...
  if (symbolic_operands == LEFT) {
    ...
    ast_ptr = add_ast_node(SUB, left_operand_ast_tc, right_operand_ast_tc, mintervals_los[left_operand_ast_tc].size(), propagated_minterval_lo, propagated_minterval_up, 1, involved_sym_inputs_cnts[left_operand_ast_tc], involved_sym_inputs_ast_tcs[left_operand_ast_tc], theory_type, smt_exprs[old_ast_tc]);
    ...
    ast_ptr = add_ast_node(SUB, left_operand_ast_tc, right_operand_ast_tc, mintervals_los[right_operand_ast_tc].size(), propagated_minterval_lo, propagated_minterval_up, 1, involved_sym_inputs_cnts[right_operand_ast_tc], involved_sym_inputs_ast_tcs[right_operand_ast_tc], theory_type, smt_exprs[old_ast_tc]);
...
uint64_t mit_bvt_engine::compute_mul(uint64_t left_operand_ast_tc, uint64_t right_operand_ast_tc, uint64_t old_ast_tc, uint8_t theory_type, uint8_t symbolic_operands) {
  ...
  uint64_t ast_ptr = add_ast_node(MUL, sym_operand_ast_tc, crt_operand_ast_tc, 1, propagated_minterval_lo, propagated_minterval_up, 1, involved_sym_inputs_cnts[sym_operand_ast_tc], involved_sym_inputs_ast_tcs[sym_operand_ast_tc], BVT, smt_exprs[old_ast_tc]);
